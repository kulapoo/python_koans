...
class AboutSets(Koan):
    def test_sets_make_keep_lists_unique(self):
        ...
        self.assertEqual({'MacLeod', 'Matunas', 'Malcolm', 'Ramirez'}, there_can_only_be_only_one)
    ...
    def test_convert_the_set_into_a_list_to_sort_it(self):
        self.assertEqual(['1', '2', '3', '4', '5'], sorted(set('12354')))

    # ------------------------------------------------------------------

    # - (difference): scotsmen - warriors ->> returns elements in scotsmen that aren't in warriors = {'Willie'}
    # | (union): scotsmen | warriors ->> combines all unique elements from both sets = {'Willie', 'Wallace', 'Leonidas', 'MacLeod'}
    # & (intersection): scotsmen & warriors ->> returns common elements = {'Wallace', 'MacLeod'}
    # ^ (symmetric difference): scotsmen ^ warriors ->> returns elements in either set but not both = {'Willie', 'Leonidas'}
    def test_set_have_arithmetic_operators(self):
        scotsmen = {'MacLeod', 'Wallace', 'Willie'}
        warriors = {'MacLeod', 'Wallace', 'Leonidas'}

        self.assertEqual({'Willie'}, scotsmen - warriors)
        self.assertEqual({'Willie', 'Wallace', 'Leonidas', 'MacLeod'}, scotsmen | warriors)
